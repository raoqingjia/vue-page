...
如果MySQL设置手动启动，每次开机都都要执行net start mysql启动mysql
<span>SQL语句</span>
SQL语句，对大小写不敏感.但是，编译的时候，操作系统会将所有字符转换成大写的，再进行编译。
如果大写，在编译的时候，可以节省转化的时间。当SQL语句大量的时候，就显得很重要了。</pre>
          <h3>MySQL的命令执行</h3>
          <pre>
...
表的最大行大小限制了列的数量（可能还有大小），因为所有列的总长度不能超过该大小。请参阅 行大小限制。
*单个列的存储要求限制了给定最大行大小内的列数。某些数据类型的存储要求取决于存储引擎，存储格式和字符集等因素。
*存储引擎可能会施加其他限制表列计数的限制。例如， InnoDB每个表的限制为1017列。
<span>行大小限制</span>
*MySQL表的内部表示具有65,535字节的最大行大小限制，即使存储引擎能够支持更大的行也是如此。 BLOB和 TEXT因为它们的内容是从该行的其余部分分开地存储列仅有助于朝向行大小限制9至12个字节。
<span>字符串</span>
