...
编译错误---只检查语法</pre>
          ...
          <pre>
java   反向代理集群
Nginx  并发能力好 Nginx不干活把请求转给应用服务器；
一个主机挂一个域名，上淘宝，页面跳转就设置及到了服务器的跳转
高并发：同一个阶段请求服务器的数量；高性能：响应的快
mysol等数据库抗受的并发量是一千
Redis 就查的快，
MQ 消息队列 例如美团订火车票，先告诉你订票成功，后短息通知你订票信息
计算种查询速度最快的是 散列表
          </pre>
          <h3>Java的编译运行过程（面试题）</h3>
          <pre>
<span>java的编译过程（常见面试题）</span>
1）编译期: .java的源文件，经过编译先生成.class字节码文件
